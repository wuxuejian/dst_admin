<div style="padding:10px;">
    <form id="easyui-form-car-office-car-register-add2">
        ...
        <ul class="ulforform-resizeable">
            <li class="ulforform-resizeable-group">
                ...
                <div class="ulforform-resizeable-input">
                    <input
                        id="easyui-form-car-office-car-register-add2-index"
                        name="car_id"
                        style="width:180px;"
                        />
                </div>
...
<div id="easyui-form-car-office-car-register-add2-uploadimage"></div>
<script type="text/javascript">
    // CarOfficeCarRegisterIndex = {
       // init: function(){
            //初始化-车辆combogrid
            $('#easyui-form-car-office-car-register-add2-index').combogrid({
                panelWidth: 350,
                panelHeight: 200,
                required: true,
                missingMessage: '请输入车牌号检索后从下拉列表里选择一项！',
                    ...
                    if(!row){ //没有选择表格行但输入有检索字符串时，提示并清除检索字符串
                        if(text && value == text){
                            $.messager.show(
                                {
                                    title: '无效值',
                                    msg:'【' + text + '】不是有效值！请重新输入车牌号检索后，从下拉列表里选择一项！'
                                }
                            );
