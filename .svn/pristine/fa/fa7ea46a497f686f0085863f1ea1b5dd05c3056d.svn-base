<div style="padding:10px 40px 20px 40px">
    <form id="easyui-form-process-car-edit" class="easyui-form" method="post">
    <input type="hidden" name="id" />
        ...
        <?php $i++; endforeach;?>
                   ...
                   <!--  <select class="easyui-combobox" id="contract_type"  name="contract_type" style="width: 173px" data-options="editable:false"  required="true"   missingMessage="请选择合同类型">
                   	...
                   	<select class="easyui-combobox" id="contract_type_id"  name="contract_type" style="width: 173px" data-options="editable:false"  required="true"   missingMessage="请选择合同类型">
	                    	...
	                    	<?php foreach($carletcontract as $val){ ?>
                            	...
                            	<option value="<?= $val['contract_type'] ?>"><?= $val['contract_type'] ?></option>
                            <?php } ?>
            ...
            <tr>
                <td>订单编号：</td>
                <td><input class="easyui-textbox" name="batch_no"  prompt="请填写订单号，如:01" value="<?php echo $result['batch_no']?>"   /></td>
                <td> 客户类型：</td>
                <td>
            ...
            <tr>
                ...
                <td> 客户方的申请人：</td>
                <td>
                   	<input class="easyui-textbox" name="shenqingren"   value="<?php echo $result['shenqingren']?>"  />
                </td>
            </tr>
            ...
            <tr>
                <td> 电话：</td>
                <td>
                    <input class="easyui-textbox" name="tel" validType="match[/((0\d{2,3})-)(\d{7,8})(-(\d{3,}))?$|1[3|4|5|7|8][0-9]\d{8}$/]" invalidMessage="电话、手机错误！" prompt="电话号码格式 区号-号码"  value="<?php echo $result['tel']?>"  />
                </td>
            </tr>
...
<script>
$('#easyui-form-process-car-edit').form('load',<?= json_encode($result); ?>);
$(function(){
//	var ajax_data =[];
	//select_type(1);
	//search(1);
...
function select_type(id)
{
	...
	if(car_brand)
	{
		var a=" <?php echo $result['car_types']['car_type'] ?>"
		console.log(a);
		var str ='';
		
		<?php foreach ($result['car_types'] as $key => $car_type): ?>
		<?php foreach ($cars[$car_type['car_brand']] as $val) : ?>
		<?php if($car_type['car_type']==$val): ?>
...
function search(id)
{
	...
	$.ajax({
		type: 'post',
		url: "<?php echo yii::$app->urlManager->createUrl(['process/car/search-number']); ?>&id=<?php echo $result['id']?>",
