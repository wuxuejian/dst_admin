<div id="echart-carmonitor-realtime-detail-dashboard" style="width:1100px;height:340px;margin:0 auto;"></div>
<div style="border:1px dashed #ddd;border-radius:4px;margin:10px;">
    ...
    <form id="easyui-from-carmonitor-realtime-detail-baseinfo" style="overflow:hidden;">
        <?php
...
<div id="echart-carmonitor-realtime-detail-battery-t" style="width:1150px;height:300px;margin:0 auto;"></div>
<div id="echart-carmonitor-realtime-detail-battery-v" style="width:1150px;height:300px;margin:0 auto;"></div>
<!-- 电机数据开始 -->
<div style="border:1px dashed #ddd;border-radius:4px;margin:10px;">
    <div style="padding:10px;font-size:18px;color:#333;font-weight:bold;">驱动电机数据</div>
    <form id="easyui-from-carmonitor-realtime-detail-moter" style="overflow:hidden;">
        <?php
            $useColumns = [
...
<div style="border:1px dashed #ddd;border-radius:4px;margin:10px;">
    ...
    <form id="easyui-from-carmonitor-realtime-detail-extremum" style="overflow:hidden;">
        <?php
            $useColumns = [
...
<div style="border:1px dashed #ddd;border-radius:4px;margin:10px;">
    ...
    <form id="easyui-from-carmonitor-realtime-detail-otherdata" style="overflow:hidden;">
        <?php
            $useColumns = [
...
<!-- 极值数据结束 -->
<script>
    var CarmonitorRealtimeDetail = new Object();
    CarmonitorRealtimeDetail.chartObject = {
        "dashboardChart": "",
        "batteryTemChart": "",
        "batteryVolChart": "",
    };
    CarmonitorRealtimeDetail.init = function(){
        try{
            this.drawDashboard(<?= json_encode($data); ?>);
            this.drawBatteryTemperature(<?= json_encode($data); ?>);
            this.drawBatteryVoltage(<?= json_encode($data); ?>);
            CarmonitorRealtimeDetail.loadData(<?= json_encode($data); ?>);//装载初始化数据
        }catch(e){
            return;
        }
        this.getData();//定时获取数据
    ...
    //绘制仪表数据
    CarmonitorRealtimeDetail.drawDashboard = function(d){
        //图表对象是否已经被缓存
        if(this.chartObject.dashboardChart){
            var dashboardChart = this.chartObject.dashboardChart;
        }else{
            var dashboardChart = echarts.init(document.getElementById('echart-carmonitor-realtime-detail-dashboard'));
    ...
    //绘制电池温度数据
    CarmonitorRealtimeDetail.drawBatteryTemperature = function(d){
        //图表对象是否已经被缓存
        if(this.chartObject.batteryTemChart){
            var chat = this.chartObject.batteryTemChart;
        }else{
            var chat = echarts.init(document.getElementById('echart-carmonitor-realtime-detail-battery-t'));
    ...
    //绘制电压数据
    CarmonitorRealtimeDetail.drawBatteryVoltage = function(d){
        //图表对象是否已经被缓存
        if(this.chartObject.batteryVolChart){
            var chat = this.chartObject.batteryVolChart;
        }else{
            var chat = echarts.init(document.getElementById('echart-carmonitor-realtime-detail-battery-v'));
    ...
    //定时获取数据
    CarmonitorRealtimeDetail.timer = 0;
    CarmonitorRealtimeDetail.getData = function(){
        if(this.timer){
        ...
        this.timer = setInterval(function(){
            $.ajax({
                    "type": "post",
                    "url": "<?= yii::$app->urlManager->createUrl(['carmonitor/realtime/detail']); ?>",
                    "data": {"car_vin": "<?= $carVin;?>"},
                    "dataType": "json",
                    "success": function(d){
                        try{
                            CarmonitorRealtimeDetail.drawDashboard(d);
                            CarmonitorRealtimeDetail.drawBatteryTemperature(d);
                            CarmonitorRealtimeDetail.drawBatteryVoltage(d);
                            CarmonitorRealtimeDetail.loadData(d);
                        }catch(e){
                            return;
                        }
                    }
    ...
    //装载显示数据
    CarmonitorRealtimeDetail.loadData = function(d){
        var data = d;
        //数据格式化
        ...
        $('#easyui-from-carmonitor-realtime-detail-baseinfo').form('load',data);
        $('#easyui-from-carmonitor-realtime-detail-moter').form('load',data);
        $('#easyui-from-carmonitor-realtime-detail-extremum').form('load',data);
        $('#easyui-from-carmonitor-realtime-detail-otherdata').form('load',data);
    }
    CarmonitorRealtimeDetail.init();
</script>