...
/*        'host'=>'120.76.114.155',
    ...
    'logFile'=>dirname(__FILE__).'/dailyChargeStatistics.php.log',//日志文件
];
...
/*
 * 按日统计“APP”或“电卡”的充电量/充电金额/充电次数，并保存到数据库中。
 * @$dbhSystem：数据库连接资源
 ...
 */
function dailyChargeStatistics($dbhSystem,$dbCloucdz,$logicAddr,$type){
    switch($type){
        case 'app':
            $cardNoCon = " `START_CARD_NO` LIKE '999000%' ";
            $tableName = 'cs_report_charge_statistics_app';
            break;
        case 'card':
            $cardNoCon = " `START_CARD_NO` NOT LIKE '999000%' ";
            $tableName = 'cs_report_charge_statistics_card';
            break;
    }
    if(!isset($cardNoCon)){
        die;
    ...
    if($records){
        foreach($records as $record){
            ...
            $dailyArr[$yearMonth][$day] = $record['chargeKwh'] . '|' . $record['chargeMoney'] . '|' . $record['chargeTimes'];
        }
    }
    //print_r($dailyArr);exit;
    if($dailyArr){
        //遍历每一个年月的充电情况，向数据表中插入或更新纪录，并更新该月合计金额
...
function main(){
    ...
    //---每日统计“APP”和“电卡”的充电量/充电金额/充电次数-------
    dailyChargeStatistics($dbhSystem,$dbCloucdz,$logicAddr,'app');
    dailyChargeStatistics($dbhSystem,$dbCloucdz,$logicAddr,'card');
