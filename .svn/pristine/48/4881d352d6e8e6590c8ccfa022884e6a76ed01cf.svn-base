<div style="padding:15px"> 
    <form id="easyui-form-car-baseinfo-driving-license-edit">
        ...
        <table cellpadding="8" cellspacing="0">
            ...
            <tr>
             ...
             <tr>
                    ...
                    <td>
                        <ul style="padding:0;margin:0;list-style:none;overflow:hidden;" id="process-repair-uploadfile-edit">
                            <?php
                                $thumbs = [
                                    ['image','行驶证照片'],
                                ];
                            ...
                            ?>
                                <li style="float:left;margin-right:16px;position:relative;cursor:pointer" >
                                    <div style="width:100px;height:100px;">
                                        <img  id="<?php echo $item[0]; ?>"  class="repairImg" src="<?php echo !empty($licenseInfo['image']) ? $licenseInfo['image']:'./images/add.jpg'; ?>" width="100" height="100" />
                   ...
                   <td><?php if(!empty($licenseInfo['image'])):?><span><a href="<?php echo $licenseInfo['image']?>" target="_blank" style="text-decoration:none">查看大图</a></span><?php endif;?></td>
                </tr>
        </table>
    </form>
</div>
<iframe id="iframe-process-repair-uploadimage" name="iframe-process-repair-uploadimage" style="display:none;"></iframe>
<div id="easyui-dialog-process-repair-uploadimage-edit"></div>
<script>
    var oldData = <?php echo json_encode($licenseInfo); ?>;
    //var oldData = <?php echo json_encode($drivingLicense); ?>;
    //oldData.addr = oldData.addr > 0 ? formatDateToString(oldData.addr) : '';
    oldData.register_date = oldData.register_date > 0 ? formatDateToString(oldData.register_date) : '';
    
    oldData.issue_date = oldData.issue_date > 0 ? formatDateToString(oldData.issue_date) : '';
    //oldData.archives_number = oldData.archives_number > 0 ? formatDateToString(oldData.archives_number) : '';
    //oldData.total_mass = oldData.total_mass > 0 ? formatDateToString(oldData.total_mass) : '';
    oldData.force_scrap_date = oldData.force_scrap_date > 0 ? formatDateToString(oldData.force_scrap_date) : '';
    oldData.valid_to_date = oldData.valid_to_date > 0 ? formatDateToString(oldData.valid_to_date) : '';
    //oldData.image = oldData.image > 0 ? formatDateToString(oldData.image) : '';

    $('#easyui-form-car-baseinfo-driving-license-edit').form('load',oldData);

    var ProcessRepairUpload = new Object();
    ProcessRepairUpload.init = function(){
        //初始化照片上传窗口
        $('#easyui-dialog-process-repair-uploadimage-edit').dialog({
            title: '照片上传',   
            width: 500,   
            height: 160,   
            closed: false,   
            ...
            buttons: [{
                ...
                iconCls:'icon-cancel',
                handler:function(){
                    $('#easyui-dialog-process-repair-uploadimage-edit').dialog('close');
                }
            }],
            onClose: function(){
                $(this).window('clear');
        ...
        //给上传故障图片绑定各类事件
        $('#process-repair-uploadfile-edit').children('li')
            .click(function(){ //单击打开上传窗口
                var columnName = $(this).find('input').attr('name');
                $('#easyui-dialog-process-repair-uploadimage-edit')
                    .dialog('open')
    ...
    ProcessRepairUpload.uploadComplete = function(rData){
        if(rData.status){
            $('#easyui-dialog-process-repair-uploadimage-edit').dialog('close');
            var inputControl = $('#process-repair-uploadfile-edit').find('input[name='+rData.columnName+']');
