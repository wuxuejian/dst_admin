<div class="easyui-tabs" data-options="fit:true,border:false"> 
    ...
    <div title="车辆基本信息" style="padding:15px">
        <form id="easyui-form-car-baseinfo-edit" method="post">
            ...
            <table cellpadding="8" cellspacing="0">
                <tr>
                    ...
                    <td><div style="width:85px;text-align:right;">车辆状态</div></td>
                    <td>
                        <select
							...
							disabled="true"
                        >
                        <?php foreach($config['car_status'] as $val){ ?>
                ...
                <tr>
                    ...
                    <td>
                    <!-- id="car_baseinfo_edit_input_car_brand_name" -->
                     <!-- name="car_model_name" -->
                      <!-- readonly="true" -->
                        <input
                            name="car_model_name"
                ...
                <tr>
	                ...
	                <td colspan="5">
	                <?php if(!empty($carInfo['modified_type'])){
	                		$modified_type = explode(',', $carInfo['modified_type']);
	                	  }else{
	                	  	$modified_type = array();
	                	  }
	                ?>
	                ...
	                <?php foreach ($config['modified_car_type'] as $v):?>
	                	<span><input type="checkbox" name="modified_type[]" value="<?php echo $v['text']?>"  <?php if($modified_type && in_array($v['text'], $modified_type))  echo 'checked'; ?>  /><?php echo $v['text']?></span>
	                <?php endforeach;?>
	                <?php endif;?>
	                </td>
            	</tr>
...
<script>
    var CarBaseinfoEdit = {
        params: {
            oldData: <?php echo json_encode($carInfo); ?>,
        },
        init: function(){
            var oldData = this.params.oldData;
            this.params.oldData.reg_date = parseInt(oldData.reg_date) > 0 ? formatDateToString(oldData.reg_date) : '';
            this.params.oldData.leave_factory_date = parseInt(oldData.leave_factory_date) > 0 ? formatDateToString(oldData.leave_factory_date) : '';
            this.params.oldData.issuing_date = parseInt(oldData.issuing_date) > 0 ? formatDateToString(oldData.issuing_date) : '';
            setTimeout(function(){
                $('#easyui-form-car-baseinfo-edit').form('load',oldData);
                CarBaseinfoEdit.getCarModelName(oldData.car_model);
            },1000);
        },
        //获取填写车型的车辆名称
        getCarModelName: function(carBrand){
            var carBrandNameInput = $('#car_baseinfo_edit_input_car_brand_name');
            if(!carBrand){
