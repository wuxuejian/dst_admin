<div style="padding:15px">
    <form id="easyui-form-car-test-edit" method="post">
...
<script>
    var CarTestEdit = {
        init: function(){
            var easyuiForm = $('#easyui-form-car-test-edit');
            easyuiForm.find('input[name=car_id]').combogrid({
                panelWidth: 240,
                idField: 'id',
                textField: 'vehicle_dentification_number',
                url: "<?= yii::$app->urlManager->createUrl(['car/combogrid/car-list','car_id'=>$testInfo['car_id']]); ?>",
                method: 'get',
                mode: 'remote',
                rownumbers: true,
                columns: [[
        ...
        loadData: function(){
            if(this.loadDataTimes == 0){
                var oldData = <?php echo json_encode($testInfo); ?>;
                $('#easyui-form-car-test-edit').form('load',oldData);
            }
            this.loadDataTimes ++;
        }
    };
