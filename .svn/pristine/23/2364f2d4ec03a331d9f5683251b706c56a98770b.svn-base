<table id="easyui_datagrid_carmonitor_exception_alert_deal"></table> 
<div id="easyui_datagrid_carmonitor_exception_alert_deal_toolbar">
    <form id="easyui_form_carmonitor_exception_alert_deal_car_info">
        <div
            class="easyui-panel"
            title="车辆信息"
        ...
        >
            <ul class="ulforform-resizeable">
                ...
                <li class="ulforform-resizeable-group">
                    <div class="ulforform-resizeable-title">负责人</div>
                    <div class="ulforform-resizeable-input">
                        <input
                            class="easyui-textbox"
                            name="director_name"
                ...
                <li class="ulforform-resizeable-group">
                    <div class="ulforform-resizeable-title">联系方式</div>
                    <div class="ulforform-resizeable-input">
                        <input
                            class="easyui-textbox"
    ...
    >
        <div style="padding:4px;">
            <button onclick="CarmonitorExceptionAlertDeal.add()" class="easyui-linkbutton" data-options="iconCls:'icon-add'">添加处理记录</button>
            <button onclick="CarmonitorExceptionAlertDeal.remove()" class="easyui-linkbutton" data-options="iconCls:'icon-remove'">删除处理记录</button>
            <span style="color:red;padding-left:10px;">注：双击记录可修改，点击“确定”按钮才能保存修改！</span>
        </div>
    </div>
</div>
<form id="easyui_form_carmonitor_exception_alert_deal_save_data" style="display:none"></form>
<script>
    var CarmonitorExceptionAlertDeal = {
        params: {
            url: {
                getListData: "<?= yii::$app->urlManager->createUrl(['carmonitor/exception-deal/get-deal-record','cad_id'=>$cadId]); ?>",
                alertDeal: "<?= yii::$app->urlManager->createUrl(['carmonitor/exception-deal/alert-deal','cad_id'=>$cadId]); ?>"
            },
            datagrid: $('#easyui_datagrid_carmonitor_exception_alert_deal'),
            forms: {
                carInfo: $('#easyui_form_carmonitor_exception_alert_deal_car_info'),
                saveData: $('#easyui_form_carmonitor_exception_alert_deal_save_data')
            },
            carInfo: <?php echo json_encode($carInfo); ?>
        },
        init: function(){
            //获取列表数据
            this.params.datagrid.datagrid({  
                ...
                fit: true,
                border: false,
                toolbar: "#easyui_datagrid_carmonitor_exception_alert_deal_toolbar",
                pagination: false,
                loadMsg: '数据加载中...',
                striped: true,
                checkOnSelect: true,
                ...
                onDblClickRow: function(rowIndex,rowData){
                    var rows = CarmonitorExceptionAlertDeal.params.datagrid.datagrid('getRows').length;
                    for(var i = 0;i<rows;i++){
                        if(CarmonitorExceptionAlertDeal.params.datagrid.datagrid('validateRow',i)){
                            CarmonitorExceptionAlertDeal.params.datagrid.datagrid('endEdit',i);
                        }else{
                            CarmonitorExceptionAlertDeal.params.datagrid.datagrid('deleteRow',i);
                        }
                    }
                    CarmonitorExceptionAlertDeal.params.datagrid.datagrid('beginEdit',rowIndex);
                }
            });
            //载入车辆数据
            this.params.forms.carInfo.form('load',this.params.carInfo);
        },
    ...
    CarmonitorExceptionAlertDeal.init();
</script>