<table id="easyui-datagrid-car-alert-insurance-compulsory"></table> 
<div id="easyui-datagrid-car-alert-insurance-compulsory-toolbar">
    <div class="easyui-panel" title="数据检索" style="width:100%" data-options="
        iconCls: 'icon-search',
        border: false
    ">   
        <div class="data-search-form">
            <form id="search-form-car-alert-insurance-compulsory">
                <ul class="search-main">
                    ...
                    <li class="search-button">
                        ...
                        <button onclick="CarAlertInsuranceCompulsory.resetForm();return false;" class="easyui-linkbutton" data-options="iconCls:'icon-reload'">重置</button>
                    </li>
					 <li>
...
<script>
    var CarAlertInsuranceCompulsory = new Object();
    //配置项
    CarAlertInsuranceCompulsory.CONFIG = <?= json_encode($config); ?>;
    CarAlertInsuranceCompulsory.init = function(){
        //获取列表数据
        $('#easyui-datagrid-car-alert-insurance-compulsory').datagrid({  
            method: 'get', 
            url:"<?php echo yii::$app->urlManager->createUrl(['car/alert/ic-get-list']); ?>",   
            fit: true,
            border: false,
            toolbar: "#easyui-datagrid-car-alert-insurance-compulsory-toolbar",
            pagination: true,
            loadMsg: '数据加载中...',
            striped: true,
            checkOnSelect: true,
            ...
            columns:[[
                {field: 'car_status',title: '车辆状态',width: 70,align: 'center',sortable: true,
                    formatter: function(value){
                        try{
                            var str = 'CarAlertInsuranceCompulsory.CONFIG.car_status.' + value + '.text';
                            return eval(str);
                        }catch(e){
                            return value;
                        }
                ...
                },
                {
                    ...
                    formatter: function(value){
                        var insurer_company = <?= json_encode($insuranceCompanyName); ?>;
        ...
        //构建查询表单
        var searchForm = $('#search-form-car-alert-insurance-compulsory');
        searchForm.submit(function(){
            ...
            $('#easyui-datagrid-car-alert-insurance-compulsory').datagrid('load',data);
            return false;
        });
        searchForm.find('input[name=plate_number]').textbox({
            onChange: function(){
                $('#search-form-car-alert-insurance-compulsory').submit();
            }
        });
    ...
    //重置查询表单
    CarAlertInsuranceCompulsory.resetForm = function(){
        var easyuiForm = $('#search-form-car-alert-insurance-compulsory');
        easyuiForm.form('reset');
        easyuiForm.submit();
    }

    // 导出Excel
    CarAlertInsuranceCompulsory.exportIcList = function(){
        var searchConditionStr = $('#search-form-car-alert-insurance-compulsory').serialize();
        window.open("<?php echo yii::$app->urlManager->createUrl(['car/alert/export-ic-list']); ?>" + "&" + searchConditionStr);
    }

    CarAlertInsuranceCompulsory.init();
</script>