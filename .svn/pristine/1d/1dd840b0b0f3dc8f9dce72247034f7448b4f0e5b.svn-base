...
<script type="text/javascript">
	...
	for(var i=0;i<listData.length;i++) {
		var row = listData[i];
		if(row){
			var _lng = row.company_lng;
			var _lat = row.company_lat;
			var _address = row.company_addr;
			if(_lng && _lat){ //优先查经纬度
	...
	if(missedNum > 0){ 
		var _content = "<div style='font-size:12px;padding:3px 5px;background-color:yellow;border:1px solid gray;'>" +
							"<div style='height:32px;'>" + 
								"<table cellspacing=0 cellpadding=0 border=0 style='float:right;'>" + 
									"<tr><td>标注客户数：</td><td><span style='color:red;float:right;'>" + markedNum + "</span></td></tr>" + 
									"<tr><td>丢失客户数：</td><td><span style='color:red;float:right;cursor:pointer;' onclick='toggleMissedPoints()'>" + missedNum + "</span></td></tr>" + 
								"</table>" + 
							"</div>" + 
		...
		var _content = "<div style='font-size:12px;padding:3px;background-color:yellow;border:1px solid gray;'>" +
							"<div>标注客户数: <span style='color:red;'>" + markedNum + "</span></div>" +
						"</div>";
	}
	cr.addCopyright({id: 2, content: _content, bounds: bs}); 
	//===右上角添加统计信息 end========================================================================
	...
	function furtherProcessing(_point,row){	
		...
		var content = '';
		content += 	'<span style="font-size:14px;"><b>企业客户详情：</b></span><br/>' +
					"<br/>公司名称：" + row.company_name + 
					"<br/>公司地址：" + row.company_addr +
					"<br/>联系人姓名：" + row.contact_name + 
					"<br/>联系人手机：" + row.contact_mobile + 
					"<br/>联系人邮箱：" + row.contact_email;
		var label = new BMap.Label(row.company_name,{offset:new BMap.Size(20,-10)}); // 创建标签
	...
	function printTable(){
		...
		str += 	"<tr><th>No.</th><th>丢失客户</th><th>丢失原因</th></tr>";
	...
	function openNewWin(){ 
		...
		str += 	"<tr><th colspan=3>百度地图丢失标注统计--企业客户</th></tr>";
		str += 	"<tr style='background:#ccc;'><th>No.</th><th>丢失客户</th><th>丢失原因</th></tr>";
		if(missedPoints.length > 0){
			for(var i=0;i<missedPoints.length;i++){
				var item = missedPoints[i];
				str += "<tr>"; 
