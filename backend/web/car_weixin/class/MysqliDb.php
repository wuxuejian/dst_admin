...
class MysqliDb{
	...
	public function connect() {
		...
		$dbSocket = $this->_dbKey ["SOCKET"];
		//$this->_conn = mysqli_connect ( $dbHost, $dbUser, $dbPass, $dbName, $dbPort );
		
		$this->_conn = mysqli_init();
		if (! $this->_conn) {
			//throw new Exception ( 'connect to db fail: '.$dbHost.':'.$dbPort.'  '.$dbUser.':'.$dbPass.'  '.$dbName );
			throw new Exception ( 'mysqli_init fail!');
			return false;
		}
		
		if (!mysqli_real_connect($this->_conn, $dbHost, $dbUser, $dbPass, $dbName, $dbPort, $dbSocket, MYSQLI_CLIENT_FOUND_ROWS)){
			throw new Exception ( 'connect to db fail: '.$dbHost.':'.$dbPort.'  '.$dbUser.':'.$dbPass.'  '.$dbName .'  '.$dbSocket);
			return false;
		}
    ...
    public function update($sql) {
        ...
        if (!$urs) {
            throw new Exception('更新失败:' . mysqli_error($this->_conn));
        } else {
    ...
    public function autoCommit($mode = false) {
        ...
        if ($mode) {
            ...
            if($this->_autoCommitTime)
            {
            	throw new Exception('auto commit cnt is not zero when set autocommit to true');
            	return false;
            }
        } else {
