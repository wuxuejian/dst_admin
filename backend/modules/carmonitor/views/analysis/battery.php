...
>  
    <div data-options="region:'north',border:false" style="">
        <div class="data-search-form">
            <form id="search-form-carmonitor-analysis-battery">
                ...
                <ul class="search-main">
                    ...
                    <li class="search-button">
                        ...
                        <button onclick="CarmonitorAnalysisBattery.resetSearchForm();return false;" class="easyui-linkbutton" data-options="iconCls:'icon-search'">重置</button>
                    </li>
                </ul>
            </form>
    ...
    <div data-options="region:'center',title:'数据分析',border:false">
        ...
        ?>
        <div id="echart-carmonitor-analysis-battery-tem" style="width:98%;height:400px;margin:0 auto;"></div>
        <div id="echart-carmonitor-analysis-battery-vol" style="width:98%;height:400px;margin:0 auto;"></div>
    </div>  
</div>
<script>
    var CarmonitorAnalysisBattery = new Object();
    CarmonitorAnalysisBattery.init = function(){
        //查询表单自动化处理
        var easyuiForm = $('#search-form-carmonitor-analysis-battery');
        easyuiForm.submit(function(){
            $.ajax({
                "type": "post",
                "url": "<?= yii::$app->urlManager->createUrl(['carmonitor/analysis/battery']); ?>",
                "data": $(this).serialize(),
                "dataType": "json",
                "success": function(rData){
                    if(rData.status){
                        CarmonitorAnalysisBattery.drawTemData(rData.temData);
                        CarmonitorAnalysisBattery.drawVolData(rData.volData);
                    }else{
    ...
    //绘制电池温度数据
    CarmonitorAnalysisBattery.drawTemData = function(tData){
        if(!tData){
            return false;
        }
        var chat = echarts.init(document.getElementById('echart-carmonitor-analysis-battery-tem'));
    ...
    //绘制电池电压数据
    CarmonitorAnalysisBattery.drawVolData = function(vData){
        if(!vData){
            return false;
        }
        var chat = echarts.init(document.getElementById('echart-carmonitor-analysis-battery-vol'));
    ...
    //导出电池历史电池数据
    CarmonitorAnalysisBattery.exportBatData = function(){
        window.open("<?= yii::$app->urlManager->createUrl(['carmonitor/analysis/export-bat-data']); ?>");
    }
    //重置查询表单
    CarmonitorAnalysisBattery.resetSearchForm = function(){
        var easyuiForm = $('#search-form-carmonitor-analysis-battery');
        easyuiForm.form('reset');
        easyuiForm.submit();
    }
    CarmonitorAnalysisBattery.init();
</script>