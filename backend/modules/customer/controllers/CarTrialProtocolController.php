...
class CarTrialProtocolController extends BaseController
{
    ...
    public function actionGetList()
    {
        $query = CarTrialProtocol::find()
            ...
            ->andWhere([
                'ctp_customer_type'=>'COMPANY', //客户类型为“企业客户”
                'ctp_is_del'=>0
            ]);
        //查询条件
        $query->andFilterWhere(['like', 'ctp_number', yii::$app->request->get('ctp_number')]);
        $query->andFilterWhere(['like', '{{%customer_company}}.`company_name`', yii::$app->request->get('customer_name')]);
            ...
            if ($protocolModel->validate()){
                ...
                if ($protocolModel->save(false)){
                    ...
                    if (!empty($gridData)){
                        ...
                        $ctp_cCustomer_id = $protocolModel->ctp_cCustomer_id; //保存的是'企业客户ID'
                        ...
                        foreach ($gridData as $val){
                            ...
                            $protocolDetailsModel->ctpd_cCustomer_id = $ctp_cCustomer_id;
...
//                             $carUpdateStatus = Car::updateAll(['car_status' => 'INTRIAL'], ['id' => $protocolDetailsModel->ctpd_car_id, 'car_status' => 'STOCK','is_del'=>0]); //改为“试用中”
                            $statusRet = Car::changeCarStatusNew($protocolDetailsModel->ctpd_car_id, 'INTRIAL', 'customer/car-trial-protocol/add', '新建试用协议明细',['car_status'=>'STOCK','is_del'=>0]);
            ...
            if ($protocolModel->validate()){
                ...
                if ($protocolModel->save(false)){
					...
					if (!empty($changedRows)){
                        ...
                        $ctp_id = $protocolModel->ctp_id;
                        $ctp_cCustomer_id = $protocolModel->ctp_cCustomer_id;
...
// 								$carUpdateStatus = Car::updateAll(['car_status'=>'INTRIAL'],['id'=>$carId,'car_status'=>'STOCK','is_del'=>0]); //改为“试用中”
								$statusRet = Car::changeCarStatusNew($carId, 'INTRIAL', 'customer/car-trial-protocol/edit', '确定保存车辆 （修改协议窗口、试用车管理窗口）',['car_status'=>'STOCK','is_del'=>0]);
								if($isSavedNewCar && $statusRet['status']){
									$transaction->commit();  //提交事务
                                    $addCar_success[] = $plate_number;
								}else{
...
//                     $carUpdateStatus = Car::updateAll(['car_status'=>'INTRIAL'],['id'=>$carId,'car_status'=>'STOCK','is_del'=>0]); //改为“试用中”
                    ...
                    $statusRet = Car::changeCarStatusNew($carId, 'INTRIAL', 'customer/car-trial-protocol/save-cars', '确定保存车辆 （修改协议窗口、试用车管理窗口）',['car_status'=>'STOCK','is_del'=>0]);
                    if($isSavedNewCar && $statusRet['status']){
                    	if($car['car_status2'] == 'REPAIRING'){
                    		$repairing = true;
                    	}
    ...
    {	
        ...
        $protocolDetails = CarTrialProtocolDetails::find()
                        ->select(['ctpd_id','ctpd_protocol_id','ctpd_car_id','ctpd_cCustomer_id'])
        ...
        if($protocolDetails){
            ...
            Car::changeCarStatusNew($carIds, 'STOCK', 'customer/car-trial-protocol/back-car', '归还车辆 （修改协议窗口、试用车管理窗口）',['car_status'=>'INTRIAL']);
    ...
    public function actionExportWidthCondition()
    {
        ...
        header("Content-Disposition: attachment; filename=".iconv('utf-8','gbk','企业客户试用协议列表.xls'));
