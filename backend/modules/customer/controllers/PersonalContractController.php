...
class PersonalContractController extends BaseController
{
    ...
    public function actionGetList()
    {
        ...
        $query = CarLetContract::find()
                ...
                ->andWhere([
                    '{{%car_let_contract}}.customer_type'=>'PERSONAL', //客户类型为“个人客户”
    ...
    public function actionAdd()
    {
        ...
        if(yii::$app->request->isPost){
            ...
            $model->pCustomer_id = yii::$app->request->post('pCustomer_id'); //保存的是'个人客户ID'
            ...
            if($model->validate()){
                ...
                if($model->save(false)){
                    ...
                    foreach($carInfo as $val){
                        ...
                        $carLetRecordModel->pCustomer_id = $model->pCustomer_id;
                        ...
                        if(!$carLetRecordModel->validate()){
                            ...
                            if($errors){
                                foreach($errors as $val){
                                    $returnArr['info'] .= "车辆：{$val['plate_number']}签约失败,{$val[0]}！";
                                }
                            }else{
                        ...
                        //验证通过
                        $carLetRecordModel->let_time = time();
                        if($carLetRecordModel->save(false)){
                            //更新车辆状态出租中
                        	$statusRet = Car::changeCarStatusNew($carLetRecordModel->getAttribute('car_id'), 'LETING', 'customer/personal-contract/add', '新建合同(个人)',['car_status'=>'STOCK','is_del'=>0]);
                        	if(!$statusRet['status']){
                    ...
                    UserLog::log("新建个人客户合同(合同编号:".$model->number.")",'sys');
                }else{
                    $returnArr['status'] = false;
    ...
    public function actionEdit()
    {
        ...
        if(yii::$app->request->isPost){
            ...
            //echo '<pre>';
            //var_dump($model->contract_type);exit;
            $returnArr = [];
            $returnArr['status'] = true;
            $returnArr['info'] = '';
            if($model->validate()){
                ...
                if($model->save(false)){
                    ...
                    //添加合同修改记录日志
                    UserLog::log("修改个人客户合同(合同编号:".$model->number.")",'sys');
                }else{
                	$transaction->rollback(); //回滚事务
                    $returnArr['status'] = false;
        ...
        $model = CarLetContract::findOne(['id'=>$id]);
        $model or die('record not found');
        $companyCustomer = CustomerPersonal::find()
                            ->select(['id','id_name'])
                            ->where(['is_del'=>0])
                            ->asArray()->all();
        //查询当前合同签约的所有没有被归还的车辆
    ...
    public function actionAddEditCar()
    {
        ...
        if (!empty($data['plate_number'])) {
            // 检查新增的各车辆是否为“库存”状态。若是库存则再检查是否已添加商业险信息（在有效期内）。
            $returnData = $this->checkCarStockStatusAndBusinessInsurance($_POST['plate_number'],$data['id']);
            if(is_array($returnData) && !empty($returnData)){
                return json_encode($returnData); //不是库存车辆，直接退出
            }elseif(is_string($returnData) && $returnData != ''){
                $noBusinessInsuranceTip = $returnData; //提示车辆无商业险
        ...
        if(isset($data['id']) && is_array($data['id'])){
            foreach($data['id'] as $key=>$val){
                if($val == 0){
                    ...
                    $model->pCustomer_id = $data['pCustomer_id'];
                ...
                if($model->validate()){
                    ...
                    if($model->scenario == 'default'){
                        ...
                        $statusRet = Car::changeCarStatusNew($model->car_id, 'LETING', 'customer/personal-contract/add-edit-car', '批量添加或修改签约车辆(个人)',['car_status'=>'STOCK','is_del'=>0]);
    ...
    public function actionCarBack()
    {
        ...
        if($carLetRecord){
            ...
            $statusRet = Car::changeCarStatusNew($carIds, 'STOCK', 'customer/personal-contract/back-car', '签约车辆归还(个人)',['car_status'=>'LETING']);
            if(!$statusRet['status']){
            	return json_encode(['status'=>false,'info'=>$statusRet['info']]);
            }
            
            CarLetRecord::updateAll(['back_time'=>time()],['in','id',$id]);
            //添加车辆归还日志
        ...
        if(yii::$app->request->isPost){
            ...
            if($model->validate()){
                ...
                if(!CarLetContract::updateAll(['cost_expire_time'=>$model->cost_expire_time],['id'=>$model->contract_id])){
                    ...
                    $model->pCustomer_id = $contractInfo['pCustomer_id'];
                    if($model->save(false)){
                        $returnArr['status'] = true;
    ...
    public function actionExportWidthCondition()
    {
        ...
        $lineData = [
            ...
            ['content'=>'所属运营公司','font-weight'=>true,'width'=>'15'],
        ...
        header("Content-Disposition: attachment; filename=".iconv('utf-8','gbk','个人客户合同订单列表.xls'));
