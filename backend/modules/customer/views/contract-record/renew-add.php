<form id="easyui-form-customer-contract-record-renew-add" class="easyui-form">
...
<div class="easyui-panel" title="当前出租车辆" style="width:100%;" data-options="
        ...
        border: false
    "></div>
<table id="easyui-datagrid-car-contract-record-renew-add"></table>
<script>
    var CarContractRecrdRenewAdd = new Object();
    CarContractRecrdRenewAdd.init = function(){
        $('#easyui-datagrid-car-contract-record-renew-add').datagrid({  
            border: false,
            pagination: false,
            loadMsg: '数据加载中...',
            striped: true,
        ...
        var datagrid = $('#easyui-datagrid-car-contract-record-renew-add');
        <?php foreach($letCar as $val){ ?>
        datagrid.datagrid('appendRow',<?php echo json_encode($val); ?>);
        <?php } ?>
    }
    CarContractRecrdRenewAdd.init();
</script>