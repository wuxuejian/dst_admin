...
<script type="text/javascript">
	...
	for(var i=0;i<listData.length;i++) {
		var row = listData[i];
		if(row){
			var _lng = row.personal_lng;
			var _lat = row.personal_lat;
			var _address = row.id_address;
			if(_lng && _lat){ //优先查经纬度
	...
	function furtherProcessing(_point,row){	
		...
		var content = '';
		content += 	'<span style="font-size:14px;"><b>个人客户详情：</b></span><br/>' +
					"<br/>姓名：" + row.id_name + 
					"<br/>性别：" + (row.id_sex==1 ? '男' : '女') + 
					"<br/>手机：" + row.mobile + 
					"<br/>邮箱：" + row.email + 
					"<br/>地址：" + row.id_address;
		var label = new BMap.Label(row.id_name,{offset:new BMap.Size(20,-10)}); // 创建标签
	...
	function openNewWin(){ 
		...
		str += 	"<tr><th colspan=3>百度地图丢失标注统计--个人客户</th></tr>";
