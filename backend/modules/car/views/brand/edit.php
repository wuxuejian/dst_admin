<div style="padding:10px 20px">
    <form id="easyui-form-car-brand-edit">
...
<script>
    var CarBrandEdit  = {
        init: function(){
            var easyuiForm = $('#easyui-form-car-brand-edit');
            easyuiForm.find('input[name=pid]').combogrid({
                ...
                idField: 'id',
                textField: 'name',
                url: "<?= yii::$app->urlManager->createUrl(['car/combogrid/car-brand','id'=>$brandInfo['id']]); ?>",
                method: 'get',
                scrollbarSize:0,
                pagination: true,
                pageSize: 10,
        ...
        loadData: function(){
            if(this.loadDataTimes == 0){
                var easyuiForm = $('#easyui-form-car-brand-edit');
                easyuiForm.form('load',<?= json_encode($brandInfo); ?>);
            }
            this.loadDataTimes ++;
        }
    };
