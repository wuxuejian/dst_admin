...
<table id="easyui-datagrid-process-car-transfer2-details-add"></table>
<!-- toolbar start -->
<div id="process-car-transfer2-details-add-toolbar">
    <form id="easyui-form-process-car-transfer2-details-add" class="easyui-form" method="post">
		<input type="hidden" name="transfer_list_id" value="<?=$transfer_list_id?>"/>
		...
		<tr>
			...
			<td>
				<input
					id="easyui-form-process-car-transfer2-details-add-combogrid"
					name="car_id"
					style="width:180px;"
					/>
			</td>
...
<!-- 窗口 -->
<script>
    var ProcessCarTransfer2DetailsAdd = new Object();
	ProcessCarTransfer2DetailsAdd.CarBrandSelect = function(rec){
		var oc  = rec.id;
		//加载车型
		$('#process-car-transfer2-details-add-car-type').combobox({
    ...
    //提交表单
    ProcessCarTransfer2DetailsAdd.submitForm = function(){
        var form = $('#easyui-form-process-car-transfer2-details-add');
        if(!form.form('validate')){
            return false;
        }
		form.ajaxSubmit({
			url: "<?php echo yii::$app->urlManager->createUrl(['process/car-transfer/details-add']); ?>",
			dataType : "json",
	...
	//初始化-车辆combogrid
	$('#easyui-form-process-car-transfer2-details-add-combogrid').combogrid({
		panelWidth: 450,
		panelHeight: 200,
		required: true,
		...
		url: '<?= yii::$app->urlManager->createUrl(['car/stock/get-cars-by-add']); ?>&car_type_id='+<?=$carTransferListInfo['car_type_id']?>,
		method: 'get',
		scrollbarSize:0,
		pagination: true,
		pageSize: 10,
