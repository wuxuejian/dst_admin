...
<script>
    ...
    function transPosition(startIndex){
        ...
        for(var i = startIndex; i <= startIndex + 99;i++){
            if(trackData[i]){
                transIndex = i;
				if(trackData[i].longitudeValue=='0.0000000' || trackData[i].latitudeValue=='0.0000000' || trackData[i].longitudeValue==null || (trackData[i].longitudeValue<100 && trackData[i].latitudeValue>100)){	//过滤经纬度为0的
					trackData[i].longitudeValue='118.0000000';
					trackData[i].latitudeValue='11.0000000';
					//continue;
				}
    ...
    function transComplete(){
        for (var i in trackData) {
            ...
            var pt = new BMap.Point(transTrackData[i].longitudeValue,transTrackData[i].latitudeValue);
            markers.push(new BMap.Marker(pt));
            var str = '<div style="text-align:left;">'
                + '&nbsp; &nbsp;车牌号：'+trackData[i].plate_number
                + '<br />&nbsp; &nbsp;车架号：'+trackData[i].carVin
                + "<br />采集时间："+window.parent.formatDateToString(trackData[i].collectionDatetime,true)
                + "<br />&nbsp; &nbsp; &nbsp;&nbsp;车速："+trackData[i].speed+"km/h"
                + "<br />&nbsp; &nbsp; &nbsp;&nbsp;SOC："+trackData[i].soc+"%";
                + '</div>';
           data_info.push([
                transTrackData[i].longitudeValue,
                transTrackData[i].latitudeValue,
                str
            ]);
        }
