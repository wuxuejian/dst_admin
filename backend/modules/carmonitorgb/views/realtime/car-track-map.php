...
<script type="text/javascript">
    ...
    function transPosition(startIndex){
        ...
        for(var i = startIndex; i <= startIndex + 99;i++){
            if(trackData[i]){
                transIndex = i;
                str += trackData[i].longitudeValue+','+trackData[i].latitudeValue+';';
            }else{
                break;
            }
        }
            ...
            success: function (rData) {
                if(rData.status == 0){
                    for(var i in rData.result){
                        transTrackData.push({
                            'longitudeValue': rData.result[i].x,
                            'latitudeValue': rData.result[i].y,
                        });
                    }
    ...
    function drawMap(){
        ...
        var point = new BMap.Point(transTrackData[0].longitudeValue, transTrackData[0].latitudeValue);
    ...
    /*----绘制覆盖物开始----*/
    function addCover(){
        var startPoint = new BMap.Point(transTrackData[0].longitudeValue,transTrackData[0].latitudeValue);
        ...
        var endPoint = new BMap.Point(transTrackData[dataLength - 1].longitudeValue,transTrackData[dataLength - 1].latitudeValue);
    ...
    function drawLuShu(){
        ...
        for(var i in transTrackData){
            var point = new BMap.Point(transTrackData[i].longitudeValue,transTrackData[i].latitudeValue);
            arrPois.push(point);
            <?php if($trackCatch){ ?>
            landmarkPois.push({
                lng:transTrackData[i].longitudeValue,
                lat:transTrackData[i].latitudeValue,
                html:'<div style="text-align:left;">&nbsp; &nbsp;车牌号：<?php echo $plate_number; ?>'
                +'<div style="text-align:left;">&nbsp; &nbsp;车架号：'+trackData[i].carVin
                +'<div style="text-align:left;">采集时间：'+window.parent.formatDateToString(trackData[i].collectionDatetime,true)
                +'<br />当前速度：'+trackData[i].speed+'km/h'
                +'<br />&nbsp; &nbsp; &nbsp; SOC：'+trackData[i].soc+'%'
                +'<br />&nbsp; &nbsp; &nbsp; 经度：'+trackData[i].longitudeValue
                +'<br />&nbsp; &nbsp; &nbsp; 纬度：'+trackData[i].latitudeValue,
