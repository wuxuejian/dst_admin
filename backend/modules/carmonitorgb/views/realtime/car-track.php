<div class="easyui-layout" data-options="{fit:true,border:false}">
    <div data-options="region:'west',title:'条件筛选',border:false" style="width:240px;">
        <div id="carmonitorgb_realtime_car_track_calendar" style="margin:10px auto;"></div>
        <div id="carmonitorgb_realtime_car_track_hour_mark">
        <?php
        $hasDataIndex = 0;
        foreach($threeDayHasDataHour as $k=>$v){
            ...
            for($i = 0;$i<=23;$i++){
                $i = $i < 10 ? '0'.$i : $i;
                if(isset($v[$i])){
                    echo '<li class="has-data" hasdataindex="'.$hasDataIndex.'" value="'.$v[$i].'" style="border:1px solid #25a825;width:20px;height:20px;line-height:20px;text-align:center;float:left;margin:0 4px 4px 0;color:#25a825;cursor:pointer;" onclick="CarmonitorgbRealtimeCarTrack.setMark(this)" >'.$i.'</li>';
                    $hasDataIndex ++;
                }else{
        ...
        <form class="data-search-form" id="search-form-carmonitorgb-realtime-car-track">
                    ...
                    <div class="item-input" style="width:150px;">
                        <select
                            ...
                            data-options="{panelHeight: 'auto',editable:false,onChange: function(){
                                CarmonitorgbRealtimeCarTrack.search();
                            }}"
                        >
    ...
    <div data-options="region:'center',title:'',border:false," style="padding:5px;background:#eee;"><iframe id="iframe-carmonitorgb-realtime-car-track" style="width:100%;height:100%;" frameborder="none"></iframe></div>
</div>
<script>
    var CarmonitorgbRealtimeCarTrack = {
        params: {
            url: {
        ...
        init: function(){
            var searchForm = $('#search-form-carmonitorgb-realtime-car-track');
            searchForm.submit(function(){
                var iframe = document.getElementById('iframe-carmonitorgb-realtime-car-track');
                $(iframe.contentWindow.document.body).html('');
                $(iframe).attr('src',"<?php echo yii::$app->urlManager->createUrl(['carmonitorgb/realtime/car-track-map']); ?>"+'&'+$(this).serialize());
                return false;
            });
            //日历
            $('#carmonitorgb_realtime_car_track_calendar').calendar({
                width:200,
                height:160,
                ...
                onSelect: function(date){
                    ...
                    easyuiWindow.window('refresh',CarmonitorgbRealtimeCarTrack.params.url.carTrack+'&day='+date+'&car_vin=<?= $carVin; ?>');
                },
           ...
           // }); 
            ...
            //自动查询
            var hasDataLi = $('#carmonitorgb_realtime_car_track_hour_mark')
        ...
        search: function(){
            $('#search-form-carmonitorgb-realtime-car-track').submit();
        },
        reset: function(){
            var searchForm = $('#search-form-carmonitorgb-realtime-car-track');
        ...
        setMark: function(obj){
            var hasMarkLi = $('#carmonitorgb_realtime_car_track_hour_mark').find('li.select-mark');
            ...
            switch(hasMarkLi.length){
                ...
                case 2:
                    ...
                    if(endDateIndex - startDateIndex >= 24){
                        $.messager.alert('操作失败','时间跨度不能超过24小时！','error');
                        return false;
                    }
                    searchForm.find('input[name=start_date]').val(startDate);
    ...
    CarmonitorgbRealtimeCarTrack.init();
</script>