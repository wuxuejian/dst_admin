...
require_once dirname(dirname(dirname(dirname(__FILE__)))). '/db/MongoDBNew.php';
...
//         }
        ...
        if($this->saveData['speed'] > 0){
        	$this->saveData['car_current_status'] = 1;
        }
...
//         if($message['commandSingle'] != 0x05){
            //非补发消息
            //如果车辆提交的里程为0则不更新里程信息
            if(empty($this->saveData['total_driving_mileage']) || $this->saveData['total_driving_mileage']==429496729.5){
            ...
            if($this->saveData['latitude_value'] == 0 || $this->saveData['longitude_value'] == 0){
            	unset($this->saveData['latitude_value']);
            	unset($this->saveData['longitude_value']);
            }
...
//         }
        ...
        //写入国标数据库开始...
			$newData = [
				...
				'companyNo' => 3	//企业编号，1福嘉太，2G7，3北汽(地标)，4东风（地标）
			];
